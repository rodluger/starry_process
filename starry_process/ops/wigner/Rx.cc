...
int APPLY_SPECIFIC(Rx)(PyArrayObject *input0,   // theta
  ...
  shape = &(shape_vec[0]);
  auto Rx_out = allocate_output<DO0>(ndim, shape, TO0, output0, &success);
  auto dRxdtheta_out =
      allocate_output<DO1>(ndim, shape, TO1, output1, &success);
  if (success) {
    return 1;
  }
  Map<Vector<DO0, SP__NWIG>> Rx(Rx_out);
  Map<Vector<DO1, SP__NWIG>> dRxdtheta(dRxdtheta_out);

  // Compute!
  computeRx(theta, Rx, dRxdtheta);

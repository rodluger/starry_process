...
Figure~\ref{fig:mean_normalization}. \xxx{As it has been pointed out before \citep{?},
...
\xxx{While this is a formally perfect degeneracy for a single light curve, we can
    ...
    NO WE CAN'T. Talk about this in more detail.}
...
\xxx{%
    \begin{itemize}
        \item Discuss \citet{Morris2020} on ABC to measure spot sizes as a
              function of spectral type.
              Brett uses the ``smoothed amplitude'', the difference between the
              maximum and minimum flux in a de-trended light curve, as the summary statistic
              in ABC to try to infer spot coverage fractions. But this is \emph{not an
                  observable} as we discussed above. Poking around in his code reveals he
              assumes $n=3$ spots for all stars, so his results are extremely
              prior-dominated. He also fixes limb darkening and assumed a uniform
              (not isotropic) prior on the inclinations. But there's probably an
              easy way to turn his ABC code into a GP, since his expression for the flux
              is closed-form and (probably) easily integrable.
              Also note that his contrast $c'$ is defined in the opposite sense
              as ours (our $c = 1 - c'$.)
        \item Discuss \citet{Basri2020}, how we agree and disagree.  Gibor has some very
              good points about the null space, but he is overly pessimistic about what
              can and can't be learned from light curves. He does not consider the
              power of ensemble analysis, and claims there's just no way to disentangle
              spot evolution from differential rotation.
        \item Discuss \citet{Jackson2013}.
        \item Discuss \citet{Mazeh2015}.
    \end{itemize}
}
...
\begin{enumerate}
    ...
    \item \emph{Keep in mind the polar spot degeneracy}. Even when modeling
          ...
          variance. \xxx{Or an isotropic prior on $\mu_\phi$.}

...
See Figure~\ref{fig:nullspace_ensemble}.
...
\begin{figure}[ht!]
    \begin{centering}
        ...
        \oscaption{nullspace_ensemble}{%
            ...
            stellar light curve at a single inclination (thin blue curves),
            at 3 different inclinations (thin orange curves), and at 10 different
            inclinations (thin green curves). The thicker curves correspond to
            the mean shrinkage for each experiment. The inclinations are
            drawn randomly from an isotropic distribution.
...
\xxx{More soon. Include likelihood function.}
...
%
\begin{align}
    \tilde{\mathbf{f}} \sim \mathcal{N}\left( \mathbf{1}, \tilde{\pmb{\Sigma}} \right)
    \quad.
\end{align}
%
This is appropriate as long as the quantity $z \ll 1$, for which the true
distribution of $\tilde{\mathbf{f}}$ is approximately Gaussian. In practice,
...
\begin{proof}{}
    ...
    \sigma &= \frac{\sin\mu}{
        \sqrt{1
            - \alpha
            + \beta
            + \left(\beta - 1\right) \cos\mu
            + \dfrac{\alpha - 1}{\cos\mu^2}}}
    \quad.
\end{proof}
%
...
%
\begin{proof}{}
    \alpha &= \frac{2 + 4 \sigma^2 + (3 + 8 \sigma^2) \cos\mu + 2 \cos\left( 2\mu \right) + \cos\left( 3\mu \right)}{16 \sigma^2 \cos\left( \frac{\mu}{2} \right)^4} \\
    \beta &= \frac{\cos\mu + 2 \sigma^2 (3 + \cos\left( 2\mu \right)) - \cos\left( 3\mu \right)}{16 \sigma^2 \cos\left( \frac{\mu}{2} \right)^4}
    \quad.
\end{proof}
%
...
\begin{proof}{}
    ...
    &=\resizebox{.925\hsize}{!}{$
            \frac{
                \alpha \beta \left(1 + \cos\mu\right)^3
                \sin\left(2\mu\right)^3
            }{
                \sigma
                \left(
                2 \alpha + \beta - 3 +
                \left(2 \alpha + \beta - 1\right) \cos\mu
                \right)
                \left(
                2 \left( \alpha + \beta - 1\right) +
                3 (\beta - 1) \cos\mu
                - 2 \left( \alpha - \beta - 1 \right) \cos\left( 2\mu \right)
                + \left( \beta - 1\right) \cos\left( 3\mu \right)
                \right)^2
            }
        $}
    \nonumber

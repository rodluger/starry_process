...
Figure~\ref{fig:mean_normalization}. As it has been pointed out before \citep{?},
...
\begin{figure}[t!]
    \begin{centering}
        \includegraphics[width=\linewidth]{figures/calibration_default_data.pdf}
        \oscaption{calibration}{%
            ...
            Table~\ref{tab:synthetic}. The surface maps for 50 synthetic stars are
...
\begin{figure}[t!]
    \begin{centering}
        ...
        \oscaption{calibration_default}{%
            ...
            with shape parameters drawn from the posterior in $\mu_\phi$ and $\sigma_\phi$
...
\begin{figure}[t!]
    \begin{centering}
        ...
        \oscaption{evenmodes}{%
            ...
            surface that can be inferred at some value of the inclination.
            \label{fig:evenmodes}
        }
    \end{centering}
\end{figure}
...
\begin{figure}[t!]
    \begin{centering}
        ...
        \oscaption{nullspace_ensemble_ld}{%
            ...
            stars with coefficients $u_1 = 0.5$ and $u_2 = 0.25$. Odd
            ...
            there is virtually no null space up to $l_\mathrm{max} = 15$.
...
\label{sec:tinyspots}
\xxx{TODO: The model is limited in resolution to features with radii $r \gtrsim 10^\circ$.
    Typical sun spots are MUCH smaller than that. However, our model can still
    correctly infer latitudinal parameters even when spots are really tiny
    (i.e., no bias due to the limited resolution). We might try to improve the
    numerical stability of the model at higher spherical harmonic degree, but
    that's a task for a future paper.}

\subsection{Bright spots}
\label{sec:brightspots}
\xxx{TODO: We can easily model bright spots and plages by setting $c < 0$. Show
    an example. This model can be used to differentiate between spot-dominated
    and plage-dominated surfaces, which is physically interesting. Cite
    Brett's paper on TRAPPIST-1.}

\subsection{The baseline degeneracy}
\label{sec:breaking_baseline_degeneracy}
\xxx{TODO: Discuss the baseline degeneracy and how to break it. Discuss the
    degeneracy between $c$ and $N$ and how to break it. The basic idea here
    is that if we can constrain the (spotless) photospheric temperature, we
    can uniquely infer $n$. I think this can be done by taking light curves
    in two bands, but I'm checking with Gully. @HEDGES: Anything to add on this?}

\subsection{Gaussians all the way down}
\label{sec:other-marg}
\xxx{TODO: We can also marginalize over period and limb darkening coefficients.
    Argue that it's just integrals of Wigner matrices all over again, which we
    can compute efficiently in closed form. I think we need a uniform prior on
    the rotation \emph{frequency}, which is probably fine. And I think we can
    parametrize limb darkening in terms of Kipping's $q_1$ and $q_2$ and place
    a uniform prior in that space---I'm fairly confident the integrals can still
    be solved exactly.}

\subsection{Time evolution}
\label{sec:temporal}
\xxx{TODO: The model presented in this paper assumes statics stellar surfaces,
    corresponding to perfectly periodic light curves. This is hardly ever the
    case in practice, as spots evolve and differential rotation changes the surface
    over time. We can still use this model, but we need to either restrict our analysis
    to segments of light curves of duration shorter than the timescale on which
    the surface evolves, or explicitly model the effect of time evolution on the covariance.
    The next paper will deal with temporal evolution more
    rigorously, but the way to do this is just take the elementwise product of the \starryprocess
    flux covariance with a temporal covariance. This corresponds to the Kronecker product
    in spherical harmonic space. Show a figure, give an example, etc.}

\subsection{Application to transits and RV}
\label{sec:other-work}
\xxx{TODO: The covariance in the spherical harmonic basis can be used as a prior
    in many other settings.}

\subsection{Comparison to other work}
\label{sec:other-work}
\xxx{TODO: Discuss \citet{Basri2020}, how we agree and disagree. Compare to Brett's
    paper using ABC to measure spot sizes as a function of spectral type. Discuss
    Simon Wood's paper in ecology on synthetic likelihoods. Talk about similarities
    to Gaussian Random Fields in CMB studies.}

\subsection{Caveats}
\label{sec:caveats}
\xxx{TODO: Miscellaneous caveats.}
\xxx{
    \begin{enumerate}
        \item The latitude distribution is not Gaussian, so careful
              when interpreting $\mu_\phi$ and $\sigma_\phi$
        \item The number of spots does not have to be an integer. This is
              fine, since it makes the model more flexible.
        \item Careful with light curves with large amplitudes, although
              Figure~\ref{fig:calibration_hicontrast}
              has max amplitudes of more than 10\% and works great.
        \item No differential rotation / spot evolution yet! We're working on it.
        \item We might want a shrinkage prior on sigma, since it's hard to
              break the polar spot degeneracy with single-band light curves.
    \end{enumerate}
}
...
\begin{figure}[t!]
    \begin{centering}
        ...
        \oscaption{speed}{%
            ...
            computation of the covariance matrix of the GP takes about 10ms
...
cost approaches a scaling of $K^{2.5}$, which is dominated by the factorization
...
as we discussed in \ref{sec:other-marg}, it is possible to marginalize over
...
\xxx{@DFM: Is this true? I.e., is there an efficient way to solve Equation~(\ref{eq:log-like})
    when $\pmb{\Sigma}$ is the same for all stars but $\mathbf{C}$ is different? Is there
    some limit in which an approximation of some sort can be made?}
...
\begin{enumerate}
    ...
    \item \textbf{Our GP can be used to model small, Sun-like spots.} The algorithm
          ...
          still infer their latitudinal distribution without bias.
    \item \textbf{Our GP can be extended to model time-variable surfaces.} The
          ...
          corresponding to perfectly periodic light curves. \xxx{...}
    \item \textbf{Possible applications.} Spot-dominated versus plage-dominated
\end{enumerate}

\xxx{TODO: Discuss future work, including differential rotation
    and spot evolution.}

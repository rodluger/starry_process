...
We conclude our discussion with a list of several caveats that should be kept
...
\begin{enumerate}
    \item \emph{The assumed latitude distribution is not Gaussian}.
          Careful when interpreting $\mu_\phi$ and $\sigma_\phi$.
    \item \emph{The number of spots $n$ does not have to be an integer}.
          This is fine, since it makes the model more flexible.
    \item \emph{Care should be taken when modeling large-amplitude light curves}.
          Careful with light curves with large amplitudes, although
          Figure~\ref{fig:calibration_hicontrast}
          has max amplitudes of more than 10\% and works great.
    \item \emph{Keep in mind the polar spot degeneracy}.
          We might want a shrinkage prior on sigma, since it's hard to
          break the polar spot degeneracy with single-band light curves.
    \item \emph{Careful with the data}. Jitter term, baseline, etc.
    \item \emph{Careful with sample selection}. Even non-variable light curves.
          If you're worried, use a mixture model.
\end{enumerate}

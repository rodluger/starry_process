...
\xxx{TODO: discuss the batch calibration default run.}
Stellar inclination residuals (posterior mean minus true value) normalized to the
posterior standard deviation. Only \emph{marginally} overestimate the variance.
...
\begin{figure}[p!]
    \begin{centering}
        ...
        \oscaption{calibration_ld}{%
            Same as Figures~\ref{fig:calibration_default_corner}
            and \ref{fig:calibration_default_latitude}, but for \xxx{...}
...
\xxx{More runs: 1) Unnormalized (to show what we can learn if we know the baseline, as in
    multi-band photometry. 2) Big spots. 3) Single spot. 4) Variance in everything.)}
...
\label{sec:integerspots}
Shrinkage prior on sigma. Inclination distribution is bimodal.

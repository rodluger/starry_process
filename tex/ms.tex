...
%
Let the brightness $b$ an angle $\vartheta$ away from the
center of a spot of negative unit intensity and radius $r$ be
%
\begin{align}
    b(\vartheta; r) & = \frac{1}{1 + \exp\left(\dfrac{r-\vartheta}{s}\right)} - 1
\end{align}
%
for some (constant) shape parameter $s$. In the limit $s \rightarrow 0$, $b$ approaches an
inverted top-hat function with half-width equal to $r$. For $s > 0$, each half
of $b$ is a sigmoid with half-width at half-minimum equal to $r$. For definiteness,
in our implementation of the algorithm we choose $s = 0.0033 \, \mathrm{rad}$,
which is small compared to features of interest
but not too small as to create numerical issues when computing
model gradients.

Our goal is to expand the function above in spherical harmonics. To that end,
...
%
\begin{align}
    \mathbf{b}(\pmb{\vartheta}; r) = \mathbf{B}(\pmb{\vartheta}) \, \tilde{\mathbf{s}}(r)
\end{align}
%
%
where $\mathbf{B}(\pmb{\vartheta})$ is a design matrix whose columns are
the weighted Legendre polynomials,
%
\begin{align}
    B_{k,l}(\vartheta_k) & = \sqrt{2l + 1} \, P_l\left( \cos \vartheta_k \right)
    \quad,
\end{align}
%
...
%
\begin{align}
    s_{l,m}(r) = \tilde{s}_l(r) \delta_{m,0}
    \quad.
\end{align}
%
...
%
\begin{align}
    \tilde{\mathbf{s}}(r) & = \mathbf{B}^+(\pmb{\vartheta}) \, \mathbf{b}(\pmb{\vartheta}; r)
\end{align}
%
%
where
%
\begin{align}
    \mathbf{B}^+(\pmb{\vartheta}) & = \mathbf{S} \Big(\mathbf{B}^\top(\pmb{\vartheta}) \mathbf{B}(\pmb{\vartheta}) +
    \epsilon \mathbf{I}\Big)^{-1} \mathbf{B}^\top(\pmb{\vartheta})
\end{align}
%
is the smoothed pseudo-inverse of $\mathbf{B}(\pmb{\vartheta})$ with small regularization
...
$\mathbf{B}^+(\pmb{\vartheta})$ is the exact pseudo-inverse of
$\mathbf{B}(\pmb{\vartheta})$. However,

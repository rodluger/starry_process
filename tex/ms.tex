...
\end{align}
%
are the mean and covariance of the distribution over spherical harmonic coefficients.
...
at modeling small spots. While this is probably true, a different choice of
basis would not admit the same analytic solutions to the integrals needed to
compute the GP covariance. And it is precisely the ability to compute these
integrals analytically (Appendix~\ref{sec:integrals}) that makes the evaluation
of the marginal likelihood tractable; it is difficult to envision an approach
that uses a \emph{local} basis and simultaneously enjoys the same
analyticity under integration.

In principle, small spots can be modeled with negligible ringing by simply
...
of the equations in the Appendix. However, as we showed in
...
We showed in \S\ref{sec:calibration-inference} that there exists a
strong degeneracy between the contrast of individual spots $c$ and
the total number of spots $n$ on a star. For relative photometry, in
which the true baseline level (\S\ref{sec:baseline}) is not known,
this degeneracy is perfect: relative photometry cannot tell us anything
about the total spottiness of a stellar surface. Inspection of the
...
\begin{align}
    \mu      & \propto c \, n
    \nonumber                   \\
    \sigma^2 & \propto c^2 \, n
    \quad,
\end{align}
%
respectively. If our observations were sensitive to both the true mean and
variance of stellar light curves,
we could uniquely infer $c$ and $n$. However, as we discussed in \S\ref{sec:baseline},
to very good approximation, relative photometry can only tell us about the ratio
%
\begin{align}
    \frac{\sigma^2}{\mu^2} \propto \frac{c}{n}
    \quad,
\end{align}
%
i.e., the ratio of the spot contrast to the total number of spots.
...
Fortunately, it may be possible to break this degeneracy (at least to some extent)
...
\xxx{Finally, talk about \citet{Perger2020} on interpretable GPs}.

\subsubsection{Spots and stellar variability}

\xxx{%
    \citet{Morris2020} uses the ``smoothed amplitude'', the difference between the
    maximum and minimum flux in a de-trended light curve, as the summary statistic
    in ABC. Try to infer spot coverage fractions. But this is \emph{not an
        observable}. Note that his contrast $c'$ is defined in the opposite sense
    as ours (our $c = 1 - c'$.)
}

\xxx{%
    \begin{itemize}
        \item Discuss \citet{Basri2020}, how we agree and disagree.
        \item Discuss \citep{Morris2020} on ABC to measure spot sizes as a function of spectral type.
        \item Discuss \citep{Jackson2013}.
        \item Discuss \citep{Mazeh2015}.
    \end{itemize}
}
...
\begin{enumerate}
    ...
    \item \emph{Keep in mind the polar spot degeneracy}. Even when modeling
          ...
          in which the model cannot distinguish between localized spots
...
\begin{enumerate}
    ...
    \item \textbf{Relative single-band photometry cannot uniquely constrain
          ...
          are perfectly degnerate with each other in relative single-band photometry.
          Knowledge of the

...
$\tilde{\pmb{\Sigma}}$ is always valid.
...
Lastly, the posteriors for the contrast $c$ and the number of spots $n$
...
Figure~\ref{fig:calibration_hilat}
Figure~\ref{fig:calibration_equatorial}
Figure~\ref{fig:calibration_isotropic}
Figure~\ref{fig:calibration_tinyspots}
Figure~\ref{fig:calibration_default_1}
Figure~\ref{fig:calibration_default_1000}
Figure~\ref{fig:calibration_ld}
Figure~\ref{fig:calibration_ld_1000}
Figure~\ref{fig:calibration_ld_500_no_ld}
Figure~\ref{fig:calibration_unnorm}
Figure~\ref{fig:calibration_bigspots}
Figure~\ref{fig:calibration_variance}
Figure~\ref{fig:calibration_batch_default_1000}

Tiny sunspots are about $70{,}000$ km across, comparable to large sunspots.
...
\begin{figure}[p!]
    \begin{centering}
        ...
        \oscaption{nullspace_ensemble}{%
            \emph{Top:} Posterior shrinkage as a function of spherical harmonic degree
            for a hypothetical scenario in which an observer can measure a
            stellar light curve at 1 (blue),
            3 (orange), 10 (green), and 30 (red)
            random inclinations. The thicker curves correspond to
            ...
            \emph{Middle:} Same as the top panel, but for limb-darkened
            stars with coefficients $u_1 = 0.5$ and $u_2 = 0.25$. Odd
            modes can now be probed, at the expense of the even modes.
            \emph{Bottom:} Same as the middle panel, but allowing for
            ...
            \codelink{nullspace_ensemble_ld}
...
\section{Caveats}
\label{sec:caveats}
The latitude distribution is not Gaussian. The number of spots does not have to be an integer.

\subsection{Other distributions}
\label{sec:other-dist}
Uniform spot radius distribution.

\subsection{Gaussians all the way down}
\label{sec:other-marg}
We can also marginalize over period and limb darkening coefficients.

\subsection{Prior on sigma}
\label{sec:sigma-prior}
Shrinkage prior on sigma.

\subsection{Miscellaneous}
Inclination distribution is bimodal.
...
\begin{figure}[p!]
    \begin{centering}
        \oscaption{calibration_batch_default_1000}{%
            \xxx{TODO: Batch run with $M=1000$.}

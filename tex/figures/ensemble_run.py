...
def run():
    ...
    with pm.Model() as model:
        ...
        # Set up the GP
        sp = StarryProcess(
            sa=sa, sb=sb, la=la, lb=lb, ca=ca, cb=cb, period=period
        )
        ...
        for k in range(len(flux)):
            ...
            pm.Potential(
                "sin_{:02d}".format(k), tt.log(tt.abs_(tt.sin(inc[k])))
            )
...
# DEBUG: Not ready for CI on this one
if not int(os.environ.get("ON_AZURE", 0)):
    run()

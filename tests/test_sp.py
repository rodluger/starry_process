...
def test_python():
    ...
    gp = StarryProcess(ydeg, use_theano=False)
    gp.size.set_params(size_alpha, size_beta)
    gp.contrast.set_params(contrast_mu, contrast_sigma)
    gp.latitude.set_params(latitude_alpha, latitude_beta)
    mu_python = gp.mean_ylm
    cov_python = gp.cov_ylm

    # Compare
    assert np.allclose(mu, mu_python), "error in mean"
    assert np.allclose(cov, cov_python, atol=1e-7), "error in cov"

...
def sample(runid, clobber=False, debug=False):
    ...
    optimize = inputs.get("optimize", False)
    rmin = inputs.get("spot_size_min", 10.0)
    rmax = inputs.get("spot_size_max", 45.0)
    marginalize_over_inclination = inputs.get(
        ...
        if not debug:
            ...
            with pm.Model() as model:
                ...
                r = pm.Uniform(
                    ...
                    rmin,
                    rmax,
                    testval=max(min(truth["r"], rmax - 0.01), rmin + 0.01)
                    if good_guess
                    else 0.5 * (rmin + rmax),
                )
                a = pm.Uniform(
                    "a", 0, 1, testval=truth["a"] if good_guess else 0.5
                )
                b = pm.Uniform(
                    "b", 0, 1, testval=truth["b"] if good_guess else 0.5
                )
                c = pm.Uniform(
                ...
                if marginalize_over_inclination:
                    incs = [0.0 for n in range(nlc)]
                else:
                    incs = pm.Uniform(
                        "incs",
                        0,
                ...
                if marginalize_over_inclination and np.allclose(
                    periods, periods[0]
                ):

                    # Compute the total log likelihood in a single batch
                    K = len(t)
                    gp_mean = sp.mean(t, p=periods[0])
                    gp_cov = sp.cov(t, p=periods[0])
                    gp_cov += ferr ** 2 * tt.eye(K)
                    gp_cov += baseline_var
                    cho_gp_cov = cho_factor(gp_cov)
                    R = tt.as_tensor_variable(flux.T) - tt.reshape(
                        gp_mean, (-1, 1)
                    )
                    ...
                    # Add the log likelihoods for each light curve
                    log_like = []
                    for k in range(nlc):
                        log_like.append(
                            sp.log_likelihood(
                    ...
                    log_like = tt.sum(log_like)

                pm.Potential("marginal", log_like)
                ...
                if optimize is not False:
                    # TODO: implement different methods here
                    noptim = 999
    ...
    varnames_all[3] = "cn"
    varnames_all[4] = "ccn"
    truths[3] = truth["c"] * truth["n"]
    truths[3] = truth["c"] ** 2 * truth["n"]
    ...
    if not marginalize_over_inclination:
        ...
        fig, ax = plt.subplots(
            nrows, 5, figsize=(16, 2.5 * nrows), sharex=True
        )
    ...
    for k in range(npdf):
        ...
        pdf[k] = draw_pdf(x, samples["a"][idx], samples["b"][idx],)
    fig, ax = plt.subplots(1)
    for k in range(npdf):
    ...
    if not marginalize_over_inclination:
        ...
        _period = tt.dscalar()
        _flux = tt.dvector()
        _draw = lambda flux, r, a, b, c, n, inc, period: StarryProcess(
            r=r, a=a, b=b, c=c, n=n
        ).sample_ylm_conditional(
            t, flux, ferr ** 2, baseline_var=baseline_var, p=p, i=i,
        )
        draw = theano.function(
            [_flux, _r, _a, _b, _c, _n, _inc, _period],
            _draw(_flux, _r, _a, _b, _c, _n, _inc, _period),
        )
        ydeg_inf = 15
        ...
        for n in tqdm(range(nlc)):
            ...
            for l in range(3, ydeg_true + 1, 2):
                map_true[l, :] = 0
            image_no_null[n] = 1.0 + map_true.render(
                projection="moll", res=res
            )
        ...
        for img, img_pred, show_inc, flx_pred, FILE_NAME in zip(
            ...
            fig, ax = plt.subplots(
                1 + ndraws,
                nlc,
                figsize=(12 * (1 + (nlc - 1) // 10), (1 + ndraws)),
            )
